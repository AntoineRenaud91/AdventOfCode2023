...
fn process_p1(path: impl AsRef<Path>) -> usize {
    ...
    let race_times = race_times.split_at(11).1
        .split_whitespace().map(|s| {
            s.parse::<usize>().unwrap()
        });
    let record_distances = record_distances.split_at(11).1
    .split_whitespace().map(|s| {
        s.parse::<usize>().unwrap()
    });
    race_times.zip(record_distances).fold(1usize,|result, (t_race,d_record)| {
...
fn process_p2(path: impl AsRef<Path>) -> usize {
    ...
    let t_race = t_race.split_at(11).1.replace(' ', "").parse::<usize>().unwrap();
    let d_record = d_record.split_at(11).1.replace(' ', "").parse::<usize>().unwrap();
