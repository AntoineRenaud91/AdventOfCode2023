...
fn get_position(data:&str) -> [usize;3] {
    ...
    [data_iter.next().unwrap(),data_iter.next().unwrap(),data_iter.next().unwrap()]    
}
...
impl Brick {
    fn new(data: &str) -> Self {
        ...
        if start[0]<end[0] {
            Self {start,dir:Dir::X(end[0]-start[0]+1),vlen: 0}
        } else if start[1]<end[1] {
            Self {start,dir:Dir::Y(end[1]-start[1]+1),vlen: 0}            
        } else {
            Self {start,dir:Dir::Z,vlen:end[2]-start[2]}
        }
    }
...
fn process_p1(data: &str) -> usize {
    let mut bricks = data.lines()
        .map(Brick::new).collect::<Vec<_>>();
    ...
    for (i,brick) in bricks.iter_mut().enumerate() {
        let (level, supported_by) = brick.h_pos_range().fold((0,HashSet::new()),|(mut level,mut supported_by),pos| {
            if let Some((z,s)) = level_map.get(&pos){
                match z.cmp(&level) {
                    Ordering::Equal => {supported_by.insert(*s);},
                    Ordering::Greater=> {
                        level = *z;
        ...
        brick.h_pos_range().for_each(|pos| {level_map.insert(pos, (level+1+brick.vlen,i));});
        tree.insert(i, supported_by);
    };
    tree.retain(|_,set| !set.is_empty());
    let rev_tree = tree.iter().fold(HashMap::<usize,HashSet<usize>>::new(),|mut tree, (k,set)| {
        set.iter().for_each(|l| {
...
#[test]
fn test_process_p1() {
    assert_eq!(process_p1("1,0,1~1,2,1
0,0,2~2,0,2
0,2,3~2,2,3
0,0,4~0,2,4
2,0,5~2,2,5
...
fn count_fall(tree: &HashMap<usize,HashSet<usize>>,rev_tree:&HashMap<usize,HashSet<usize>>,mut fallen:HashSet<usize>,just_fallen:HashSet<usize>) -> usize {
    fallen.extend(just_fallen.clone());
    let just_fallen = just_fallen.iter().fold(HashSet::new(),|mut just_fallen,index| {
...
fn test_count_fall(){
    ...
    let tree = HashMap::from([
        ...
        (4,HashSet::from([3])),
        (5,HashSet::from([4]))]);
    assert_eq!(count_fall(&tree, &rev_tree, HashSet::new(), HashSet::from([0])),5)
}
...
fn process_p2(data: &str) -> usize {
    ...
    (0..bricks.len()).map(|i| count_fall(&tree, &rev_tree, HashSet::new(), HashSet::from([i]))).sum()
}


#[test]
fn test_process_p2() {
    assert_eq!(process_p2("1,0,1~1,2,1
0,0,2~2,0,2
0,2,3~2,2,3
0,0,4~0,2,4
2,0,5~2,2,5
